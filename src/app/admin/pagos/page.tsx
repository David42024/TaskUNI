    ...
    <div className="space-y-6">
      ...
      <div className="card overflow-x-auto">
        <table className="w-full min-w-[900px] text-left text-sm">
          ...
          <tbody>{pagos.map((p) => <tr key={p.id_pago} className="border-b border-slate-100 last:border-0"><td className="py-2 pr-4 text-slate-900 dark:text-white">{p.usuario.nombres} {p.usuario.apellidos}</td><td className="py-2 pr-4 text-slate-500">{p.suscripcion.plan.nombre_plan}</td><td className="py-2 pr-4 text-slate-500">{formatMoney(p.monto)}</td><td className="py-2 pr-4 text-slate-500">{p.metodo_pago}</td><td className="py-2 pr-4"><span className="badge bg-brand-100 text-brand-700">{p.estado_pago}</span></td><td className="py-2 pr-4 text-slate-500">{new Date(p.fecha_pago).toLocaleDateString("es-PE")}</td></tr>)}</tbody>
        </table>
      </div>
    </div>
  );
